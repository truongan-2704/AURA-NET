"""
Validation script for AURA-NET v3.0 with full metrics
...
def parse_args():
    parser = argparse.ArgumentParser(description='Validate AURA-Net')
...
def validate(model, dataloader, device, conf_thres, iou_thres, num_classes, class_names, save_dir):
    """
    ...
    with torch.no_grad():
        for batch_idx, (imgs, targets, paths) in enumerate(tqdm(dataloader, desc='Validation')):
            imgs = imgs.to(device)
    ...
    # Plot validation batch with ground truth
    if first_batch_imgs is not None:
        plot_batch_images(
            first_batch_imgs, first_batch_labels, class_names,
            save_dir / 'val_batch0_labels.jpg'
        )

    # Plot validation batch with predictions
    if first_batch_imgs is not None and first_batch_preds is not None:
        ...
        for det in first_batch_preds:
            if len(det) > 0:
                # Convert from [x1, y1, x2, y2, score, class] to [cls, x, y, w, h]
                boxes_xyxy = det[:, :4]
                cls_ids = det[:, 5]
...
def main():
    ...
    # Setup logger
    logger = setup_logger('AURA-Net-Val', log_file=save_dir / 'val.log')
    logger.info(f'Validating AURA-Net with weights: {args.weights}')
    ...
    df.to_csv(save_dir / 'val_results.csv', index=False)

    logger.info(f'Results saved to: {save_dir}')
    logger.info(f'  - Val results CSV: {save_dir / "val_results.csv"}')
